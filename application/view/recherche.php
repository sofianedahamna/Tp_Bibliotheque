<?php 
    session_start();
require_once 'C:\wamp64\www\dossier_type_mvc\application\libraries\autoloader.php';
...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
          ...
          /**   $("output").on("click", "img[id^='btn_img_']", function(e) {
                ...
                $.ajax(ajaxOptions).done(function(tab_article) {
                    ...
                    if (tab_article) {
                        ...
                        response += `<tr class='ajust'>
                                ...
                                <td> <button> <img id='btn_img_" ${tab_article[position].categorie} "' src='../../assets/icone/icons8-loupe-32.png' alt='' data-reference=' ${tab_article[position].categorie} '></button> </td>
                            </tr>`;
                            }
           ...
           // console.log(ajaxOptions);
            $.ajax(ajaxOptions).done(function(tab_ref) {
                for (var position in tab_ref) {
                    if (tab_ref[position].categorie == "Livre") {
                        responseModal = 
                        `<div class='ajust1' data-reference=  ${tab_article[position]. categorie}  >\n\
                                                ...
                                                <p> Compositeur :   ${tab_article[position].autheur}  </p>\n\
                                                <p> Tracklist :   ${tab_article[position].resumer}  </p>\n\
                                                \n\
                                            </div>`;
                    } else if (tab_ref[position].categorie == "CD") {
                        responseModal = 
                        `<div class='ajust1' data-reference=  ${tab_article[position]. categorie}  >\n\
                                                <p> Categorie :   ${tab_article[position].categorie}  </p>\n\
                                                <p> Reference :   ${tab_article[position].reference}  </p>\n\
                    ...
                    } else if (tab_ref[position].categorie == "DVD") {
                        responseModal = 
                        `<div class='ajust1' data-reference=  ${tab_article[position]. categorie}  >\n\
                                                <p> Categorie :   ${tab_article[position].categorie}  </p>\n\
                                                <p> Reference :   ${tab_article[position].reference}  </p>\n\
                                                <p> Denomination :   ${tab_article[position].denomination} </p>\n\
                                                <p> Compositeur :   ${tab_article[position].realisateur}  </p>\n\
                                                <p> Tracklist :   ${tab_article[position].synopsis}  </p>\n\
                                                \n\
                                            </div>`;
                    }
                ...
                $("#modal-content").append(responseModal);
        ...
        function select_content_sctn(id_tab) {
                ...
                /** hydration de offcanvas pour affichage des info */
                let hydrationOffcnvsArtcl = function (tab_artcl) {
                    resetOffcnvsArtcl();
                    for (let index in tab_artcl['data_rsp']) {
                        let tabProperties = Object.getOwnPropertyNames(tab_artcl['data_rsp'][index]);
                        for (let data in tabProperties) {
                           // console.log(tabProperties[data]);
                            let label = tabProperties[data];
                           // alert(tab_artcl['data_rsp'][index][label]);
                            $("#offcnvsartcl" + label).text(tab_artcl['data_rsp'][index][label]);
                        }
                    }
                   ...
                   <!--         <li class="nav-item dropdown" id="no_load">
                            ...
                            <li class="nav-item dropdown" id="no_load">
                                ...
                                <a class="nav-link bg-light  " href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                    <span class="bg-light text-black p-1 rounded"><?= $_SESSION["nom_utlstr"] . " " . $_SESSION["prenom_utlstr"]; ?></span>
                                </a>
                                <ul id="menu_prcpl" class="dropdown-menu dropdown" aria-labelledby="navbarDropdown">
                                    <li><a class="dropdown-item" href="#">Paramètres</a></li>
                                    <li id="btn_actn_logout"><a class="dropdown-item" href="#">Déconnexion</a></li>
